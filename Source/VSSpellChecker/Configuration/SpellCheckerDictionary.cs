    ...
    public class SpellCheckerDictionary
    {
          ...
          IEnumerable<string> additionalSearchFolders)
        {
            ...
            foreach(string folder in searchFolders)
                foreach(string dictionary in Directory.EnumerateFiles(folder, "*.aff"))
                    if(File.Exists(Path.ChangeExtension(dictionary, ".dic")))
                    {
                        ...
                        if(info != null)
                            availableDictionaries[info.Name] = new SpellCheckerDictionary(info, dictionary,
                                Path.ChangeExtension(dictionary, ".dic"),
                                Path.Combine(Path.GetDirectoryName(dictionary), info.Name + "_User.dic"));
                    }
